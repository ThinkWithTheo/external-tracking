...
const StatsBar: React.FC<StatsBarProps> = ({ tasks, onTaskClick, className }) => {
  ...
  return (
    <div className={cn("space-y-4", className)}>
      ...
      {(() => {
        // Collect all subtasks in review to check if the section should be rendered
        const reviewSubtasks: ProcessedTask[] = [];
        tasks.forEach(task => {
          if (task.subtasks && task.subtasks.length > 0) {
            task.subtasks.forEach(subtask => {
              if (subtask.status.toLowerCase().includes('review')) {
        ...
        // Only render the card if there are tasks in review
            ...
            <div className="space-y-4">
              ...
              >
                ...
                <Badge variant="review" className="text-xs">
                  {reviewSubtasks.length} In Review
                </Badge>
              </div>
              
              {!isReviewCollapsed && (() => {
                // Re-process to get full details for display
                const detailedReviewSubtasks: Array<{
                  subtask: ProcessedTask;
                  parentName: string;
                  developer: string;
                  developerColor: string;
                ...
                const subtasksByDeveloper = detailedReviewSubtasks.reduce((acc, item) => {
                  const developer = item.developer;
                  if (!acc[developer]) {
                    acc[developer] = {
                      subtasks: [],
                  ...
                  acc[developer].totalHours += subtaskHours;
                  return acc;
                }, {} as Record<string, { subtasks: typeof detailedReviewSubtasks, totalHours: number, color: string }>);
      ...
      {(() => {
        // First, find all urgent subtasks to determine if the section should be rendered
        const urgentSubtasks: ProcessedTask[] = [];
        tasks.forEach(task => {
          if (task.subtasks && task.subtasks.length > 0) {
            task.subtasks.forEach(subtask => {
              ...
              if (isUrgent && !isSubtaskInProgress) {
                urgentSubtasks.push(subtask);
              }
            });
          }
        });

        // If there are no urgent tasks, don't render the card
            ...
            <div className="space-y-4">
              ...
              >
                ...
                <Badge variant="error" className="text-xs">
                  {urgentSubtasks.length} Urgent
                </Badge>
              </div>
              
              {!isUrgentCollapsed && (() => {
                // Re-process to get full details for display
                const detailedUrgentSubtasks: Array<{
                  subtask: ProcessedTask;
                  parentName: string;
                  developer: string;
                  developerColor: string;
                ...
                const subtasksByDeveloper = detailedUrgentSubtasks.reduce((acc, item) => {
                  const developer = item.developer;
                  if (!acc[developer]) {
                    acc[developer] = {
                      subtasks: [],
                  ...
                  acc[developer].totalHours += subtaskHours;
                  return acc;
                }, {} as Record<string, { subtasks: typeof detailedUrgentSubtasks, totalHours: number, color: string }>);
